...
TESTDATA = [
    ...
    ['sha256', '-E -p 0x10000', False, False],
    ['sha256', None, True, False],
    ['sha256', '-E -p 0x10000', True, True],
]
...
@pytest.mark.parametrize("sha_algo,sign_options,required,full_test",
                         TESTDATA)
def test_vboot(u_boot_console, sha_algo, sign_options, required,
               full_test):
    ...
    def test_with_algo(sha_algo, sign_options):
        ...
        make_fit('sign-images-%s-ecdsa.its' % sha_algo)
        run_bootm(sha_algo, 'unsigned images', 'dev-', True)

        # Sign images with our dev keys
        sign_fit(sha_algo, sign_options)
        # run_bootm(sha_algo, 'signed images', 'dev+', True)
        run_bootm(sha_algo, 'signed images', '+ OK', True)

        # Create a fresh .dtb without the public keys
        dtc('sandbox-u-boot.dts')

        cons.log.action('%s: Test FIT with signed configuration' % sha_algo)
        make_fit('sign-configs-%s-ecdsa.its' % sha_algo)
    ...
    def test_required_key(sha_algo, sign_options):
        """Test verified boot with the given hash algorithm.
        ...
        make_fit('sign-configs-%s-prod-ecdsa.its' % sha_algo)
        sign_fit(sha_algo, sign_options)
    ...
    try:
        ...
        cons.config.dtb = dtb
        if required:
            test_required_key(sha_algo, sign_options)
        else:
            test_with_algo(sha_algo, sign_options)
    finally:
