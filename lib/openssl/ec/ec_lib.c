                 ...
                 const EC_POINT *point, const BIGNUM *p_scalar, BN_CTX *ctx)
{
    ...
    if (group->meth->mul != NULL)
		...
		// TODO: assuming mul method is implemented, ec_wNAF_mul calls group->meth->points_make_affine,
		// which in turn calls group->meth->field_inv, which is defined as ec_GFp_simple_field_inv
		// for the nistp256 curve, and this uses the BN_priv_rand_range_ex method which
		// requires the random library. It's unnecessary as the nistp256 EC_METHOD defines
		// mul, so we can just use that method and not include the ec_wNAF_mul implementation.
		return 0;
        /* #<{(| use default |)}># */
